...
import wavelink
from database.models import PlayHistory, QueueTracks
...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        database_url = os.getenv('DATABASE_URL', 'postgresql://music_admin:123@localhost:5432/music_db')
    ...
    async def on_wavelink_track_end(self, payload: wavelink.TrackEndEventPayload):
        ...
        if not next_track:
            text_channel = discord.utils.get(player.guild.text_channels, name=DEFAULT_CHANNEL)
            if text_channel:
        ...
        # Save play history
        self.save_play_history(player.guild.id, next_track.requested_by, next_track.track_title, next_track.track_author, next_track.url)

        # Notify in channel
        text_channel = discord.utils.get(player.guild.text_channels, name=DEFAULT_CHANNEL)
        if text_channel:
            await text_channel.send(embed=track_embed(playable, next_track.requested_by, title="▶️ Now Playing"))

    def save_play_history(self, guild_id: int, user_id: int, track_title: str, track_author:str, url: str):
    ...
    def get_all_tracks_from_queue(self, guild_id: int):
            ...
            try:
                ...
                print(f"Error fetching queue tracks: {e}")
                return []
            finally:
                session.close()
                
    def save_to_queue(self, guild_id: int, track_title: str, url: str, track_author:str, requested_by: int):
            """Add track to DB queue"""
            session = self.Session()
            try:
                # Determine next position using func.max for robustness
                max_pos = session.query(func.max(QueueTracks.position))\
                                .filter_by(guild_id=guild_id)\
                                .scalar() 
    ...
    def remove_from_queue(self, guild_id: int, url: str):
        ...
        try:
            session.query(QueueTracks)\
                .filter_by(guild_id=guild_id, url=url)\
                .delete()
            session.commit()
        except Exception as e:
    ...
    def shuffle_queue(self, guild_id: int):
        ...
        session = self.Session()
        try:
            tracks = session.query(QueueTracks)\
                        .filter_by(guild_id=guild_id)\
                        .all()
            import random
    ...
    def pop_next_track(self, guild_id: int):
        ...
        session = self.Session()
        try:
            track = session.query(QueueTracks)\
                        .filter_by(guild_id=guild_id)\
                        .order_by(QueueTracks.position.asc())\
                        .first()
            if track:
            ...
            print(f"Error popping next track: {e}")
            session.rollback()
        finally:
            session.close()
    ...
    async def play(self, ctx, *, query: str = None):
        ...
        # If query is given, add it to queue
        if query:
            tracks = await wavelink.Playable.search(query)
            if not tracks:
                return await ctx.send("No tracks found!")
            track = tracks[0]
            self.save_to_queue(ctx.guild.id, track.title, track.uri, track.author, ctx.author.id)
            await ctx.send(embed=track_embed(track, ctx.author.id, title="📝 Added to Queue"))
        ...
        if not vc.playing:
            ...
            if play_track:
                ...
                track = tracks[0]
                await vc.play(track)
                self.save_play_history(ctx.guild.id, ctx.author.id, track.title, track.author, track.uri)
                await ctx.send(embed=track_embed(track, play_track.requested_by, title="▶️ Now Playing"))
            else:
    ...
    async def skip(self, ctx):
        ...
        if next_track:
            ...
            await vc.play(track)
            self.save_play_history(ctx.guild.id, ctx.author.id, track.title, track.author, track.uri)
            await ctx.send(embed=track_embed(track, next_track.requested_by, title="▶️ Now Playing"))
        else:
            await ctx.send("📭 Queue is empty!")
    ...
    async def queue(self, ctx):
        ...
        for i, track_db in enumerate(queue_tracks, 1):
            ...
            title = (track_db.track_title[:50] + '...') if len(track_db.track_title) > 50 else track_db.track_title
            queue_list.append(f"**{i}.** [{title}]({track_db.url}) (by <@{track_db.requested_by}>)")
